...
func TestDefaultFPSAndVideoProfiles(t *testing.T) {
	...
	for _, fps := range []int{5, 10, 20, 60} {
		c := cfg
		c.FPS = fps
		normalizeConfig(&c)
		if c.FPS != 15 {
...
func TestLegacyDefaultsMigration(t *testing.T) {
	...
	if a.cfg.QualityProfile != "medium" || a.cfg.BitrateKbps != 1024 || a.cfg.DefaultsRevision != 11 || a.cfg.Codec != "h264" || a.cfg.Encoder != "libx264" || a.cfg.CaptureBackend != "auto" {
...
func TestInterfaceHasOnlySupportedFPSAndBitrates(t *testing.T) {
	for _, want := range []string{`<option value="15">15 кадров/с</option>`, `<option value="25">25 кадров/с</option>`, `<option value="30">30 кадров/с</option>`} {
		if !strings.Contains(indexHTML, want) {
			t.Fatalf("FPS option is missing: %s", want)
		}
	}
	if strings.Contains(indexHTML, `<option value="10">`) || strings.Contains(indexHTML, `<option value="20">`) || strings.Contains(indexHTML, `2000 Кбит/с`) || strings.Contains(indexHTML, `data-profile="high"`) {
		t.Fatal("removed FPS or bitrate profile is still visible")
	}
}

func TestInterfaceWordingAndFavicon(t *testing.T) {
	for _, want := range []string{`href="/favicon.ico"`, `Передача трансляции в облако.`, `Ссылка для LinkVideo Server.`, `<small>1920×1080</small>`, `<small>1280×720</small>`} {
		if !strings.Contains(indexHTML, want) {
			t.Fatalf("interface text is missing: %s", want)
		}
	}
	for _, forbidden := range []string{`VLC`, `видеоплеера`, `Ссылка на этом компьютере`, `Ссылка в локальной сети`, `Имя потока`, `Передача изображения и звука компьютера в LinkVideo.`} {
...
func TestBrandUsesSameTypographyForMonitor(t *testing.T) {
	if !strings.Contains(indexHTML, `<div>LinkVideo Monitor</div>`) {
...
func TestRemoteAPIExposesManagedBusinessSettings(t *testing.T) {
	encoded, err := json.Marshal(remoteCurrentSettings{
		Protocol: "rtsp", ResolutionProfile: "full_hd", CaptureMode: "monitor", MonitorNumber: 2,
		FPS: 25, BitrateKbps: 1024, Codec: "h265", Encoder: "libx265", Cursor: true,
		PrivacyProtection: true, AudioEnabled: true, MicrophoneEnabled: true,
		MicrophoneDevice: "Office microphone", OverlayEnabled: true, LaunchWithWindows: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	text := string(encoded)
	for _, want := range []string{
		`"protocol"`, `"resolution_profile"`, `"capture_mode"`, `"monitor_number"`,
		`"fps"`, `"bitrate_kbps"`, `"codec"`, `"encoder"`, `"cursor"`,
		`"privacy_protection"`, `"audio_enabled"`, `"microphone_enabled"`,
		`"microphone_device"`, `"overlay_enabled"`, `"launch_with_windows"`,
	} {
		if !strings.Contains(text, want) {
			t.Fatalf("managed API field is missing: %s", want)
		}
	}
	for _, forbidden := range []string{`connection_link`, `remote_api_url`, `remote_control_enabled`, `restart_after_resume`} {
		if strings.Contains(text, forbidden) {
			t.Fatalf("internal API field is exposed in settings: %s", forbidden)
		}
	}
}

func TestRemoteAPISupportsBusinessCommands(t *testing.T) {
	for _, action := range []string{"start_stream", "stop_stream", "restart_stream", "restart_application"} {
		resp := remoteSyncResponse{Command: &remoteCommand{ID: json.RawMessage(`"cmd-1"`), Action: action}}
		if action == "restart_application" {
			// Execution is platform/process dependent; validate command parsing only.
			continue
		}
		if resp.Command.Action != action {
			t.Fatalf("command action changed: %s", action)
		}
	}
}
