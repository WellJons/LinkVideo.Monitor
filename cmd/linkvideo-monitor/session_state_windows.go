...
func makeSessionLockedFrame(width, height int) []byte {
	return makeWindowsStatusFrameStyled(
		...
		"Трансляция возобновится после входа в Windows",
		true,
	)
}
...
func makeWindowsStatusFrameStyled(width, height int, title, subtitle string, branded bool) []byte {
	...
	if branded {
		logoSize = clampStatusSize(height/7, 56, 126)
		logoTop = height/2 - logoSize - clampStatusSize(height/14, 30, 80)
	...
	drawStatusText(memoryDC, face, subtitle, subSize, 400, rgb(174, 181, 189), width, subTop, subTop+subSize*3)
